...
export class TraineeService implements ITraineeService {
  ...
  async getDashboardStats(traineeId: string): Promise<DashboardStats> {
    ...
    // Calculate overall progress — include both program-enrolled AND org-level courses
    let completedLectureIds = new Set(trainee.completedLectures.map(l => l.lectureId));

    // Gather all lectures from program enrollments
    let programLectureIds = new Set<string>();
    trainee.enrollments.forEach(enrollment => {
      enrollment.program.levels.forEach(level => {
        level.courses.forEach(course => {
          course.lectures.forEach(l => programLectureIds.add(l.id));
        });
      });
    });

    // Also get all org-level courses (not tied to programs) for the trainee's org
    ...
    let allLectureIds = new Set(programLectureIds);
    orgCourses.forEach(course => {
      course.lectures.forEach(l => allLectureIds.add(l.id));
    });

    const totalLectures = allLectureIds.size;
    const relevantCompleted = [...completedLectureIds].filter(id => allLectureIds.has(id)).length;

    const overallProgress = totalLectures > 0
      ? Math.round((relevantCompleted / totalLectures) * 100)
      : 0;
    ...
    // Find current course — check program courses first, then org-level courses
    ...
    // If no current course from programs, check org-level courses
    if (!currentCourse) {
      const orgCoursesWithLectures = await this.prisma.course.findMany({
        where: { organizationId: trainee.organizationId, isPublished: true },
        include: { lectures: { orderBy: { orderInCourse: 'asc' } } },
        take: 10,
      });

      for (const course of orgCoursesWithLectures) {
        const courseLectureIds = course.lectures.map(l => l.id);
        const completedCount = courseLectureIds.filter(id => completedLectureIds.has(id)).length;

        if (completedCount < courseLectureIds.length && courseLectureIds.length > 0) {
          const nextLecture = course.lectures.find(l => !completedLectureIds.has(l.id));
          currentCourse = {
            id: course.id,
            title: course.title,
            progress: Math.round((completedCount / courseLectureIds.length) * 100),
    ...
    // Calculate actual total time from all activities (more accurate than stored field)
    let calculatedTime = trainee.totalTimeOnPlatform || 0;
    const voiceMinutes = voiceSessions.reduce((sum, s) => sum + Math.round(s.durationSeconds / 60), 0);
    const simMinutes = trainee.simulationSessions.reduce((sum, s) => sum + Math.round((s.durationSeconds || 0) / 60), 0);
    const lectureMinutes = trainee.completedLectures.reduce((sum, lc) => sum + (lc.timeSpentMinutes || 0), 0);
    const activityTime = voiceMinutes + simMinutes + lectureMinutes;
    // Use the higher of stored vs calculated to avoid showing 0
    const totalTime = Math.max(calculatedTime, activityTime);

    // Calculate streak from actual activity dates
    let calculatedStreak = trainee.currentStreak || 0;
    if (calculatedStreak === 0) {
      // Check if there's any activity today or yesterday
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const hasActivityToday = voiceSessions.some(s => s.endTime >= today)
        || trainee.simulationSessions.some(s => (s.completedAt || s.startedAt) && (s.completedAt || s.startedAt)! >= today)
        || trainee.completedLectures.some(lc => lc.completedAt >= today);
      if (hasActivityToday) calculatedStreak = 1;
    }

    return {
      totalTimeOnPlatform: totalTime,
