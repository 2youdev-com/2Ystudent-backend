...
export class AIAssessmentService implements IAIAssessmentService {
  ...
  async generateFeedback(score: number, context: string): Promise<string> {
    const prompt = `
Generate constructive feedback for an electromechanical engineering trainee who scored ${score}/100 in the following context:
${context}
...
- Application of concepts`;
  ...
  private buildConversationAssessmentPrompt(
    ...
    const conversationText = history
      .map(t => `${t.speaker === 'student' ? 'Agent' : 'Client'}: ${t.message}`)
      .join('\n');

    return `
Assess this electromechanical engineering consultation conversation.
