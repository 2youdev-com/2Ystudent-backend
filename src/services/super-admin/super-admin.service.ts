...
export class SuperAdminService implements ISuperAdminService {
  ...
  async getAllOrganizations(
    ...
    const data: OrganizationSummary[] = organizations.map((org) => ({
      ...
      userCount: org.trainees.filter((t) => t.role === 'student').length,
      activeUserCount: org.trainees.filter((t) => t.role === 'student' && t.lastActiveAt && t.lastActiveAt >= thirtyDaysAgo).length,
  ...
  async getOrganizationDetails(orgId: string): Promise<OrganizationDetails> {
    ...
    return {
      ...
      userCount: org.trainees.filter((t) => t.role === 'student').length,
      activeUserCount: org.trainees.filter((t) => t.role === 'student' && t.lastActiveAt && t.lastActiveAt >= thirtyDaysAgo).length,
      trainerCount: org.trainees.filter((t) => t.role === 'supervisor').length,
      traineeCount: org.trainees.filter((t) => t.role === 'student').length,
