...
export class SimulationService implements ISimulationService {
  ...
  async processMessage(input: SimulationMessageInput): Promise<SimulationMessageOutput> {
    ...
    const conversationHistory: ConversationTurn[] = session.conversationTurns.map(t => ({
      speaker: t.speaker as 'student' | 'client',
      message: t.message,
  ...
  async analyzeSimulation(input: AnalyzeSimulationInput): Promise<SimulationAnalysisOutput> {
    ...
    const conversationMessages: ConversationMessage[] = session.conversationTurns.map(turn => ({
      speaker: turn.speaker as 'student' | 'client',
  ...
  private getSuggestedScenarios(score: number): string[] {
    if (score >= 80) {
      return ['difficult_client', 'closing_deal', 'first_contact'];
    } else if (score >= 60) {
      return ['objection_handling', 'price_negotiation', 'property_showing'];
    } else {
      return ['property_showing', 'relationship_building', 'objection_handling'];
    }
  }
  ...
  private getNextSteps(outcome: SimulationOutcome, score: number): string[] {
    ...
    if (score < 60) {
      steps.push('Review the fundamental engineering concepts covered in this session');
      steps.push('Practice with easier difficulty settings to build confidence');
    } else if (score < 80) {
      steps.push('Deepen your knowledge in the areas where the mentor challenged you');
      steps.push('Try a more advanced engineering topic');
    } else {
      steps.push('Excellent technical knowledge! Try a harder difficulty level');
      steps.push('Explore different engineering specialisation areas');
    }

    if (outcome === 'client_declined' || outcome === 'relationship_damaged') {
      steps.push('Review the session transcript to identify knowledge gaps');
    }
  ...
  private generatePerformanceSummary(score: number): string {
    if (score >= 90) return 'Outstanding performance! You demonstrated excellent engineering knowledge across all areas.';
    if (score >= 80) return 'Strong technical competency shown with room for deeper understanding in some areas.';
    if (score >= 70) return 'Good engineering knowledge demonstrated. Focus on the areas identified for improvement.';
    if (score >= 60) return 'Developing competency. Significant study needed to deepen technical understanding.';
    return 'This session highlighted important knowledge gaps. Review the fundamental concepts and try again.';
  }
  ...
  private generateRecommendations(score: number): SimulationAnalysisOutput['recommendations'] {
    ...
    if (score < 70) {
      recommendations.push({
        priority: 'high',
        category: 'review_content',
        title: 'Review Problem Solving Fundamentals',
        description: 'Your problem solving approach could be strengthened.',
        actionableSteps: [
          'Review systematic troubleshooting methodology',
          'Practice identifying root causes before proposing solutions',
          'Use structured analysis frameworks for complex problems',
        ],
      });
    }
    ...
    if (score < 80) {
      recommendations.push({
        priority: 'medium',
        category: 'practice_skill',
        title: 'Strengthen Technical Analysis',
        description: 'Improve your ability to analyse engineering scenarios thoroughly.',
        actionableSteps: [
          'Ask more clarifying questions about system parameters',
          'Summarize technical requirements before proposing solutions',
          'Pay attention to safety considerations and compliance requirements',
        ],
      });
    }
    ...
    recommendations.push({
      ...
      actionableSteps: [
        ...
        'Practice with different engineering scenarios',
        'Focus on critical thinking techniques',
      ],
    });
