...
export class ObjectionHandlingService implements IObjectionHandlingService {
  ...
  private buildFormulationPrompt(
    ...
    return `
You are ${persona.name}, an engineering mentor with a ${persona.personality} teaching style.
Background: ${persona.background}

Technical areas to probe: ${persona.objections.join(', ')}

Based on the conversation so far:
${recentTurns.map(t => `${t.speaker === 'student' ? 'Student' : 'Mentor'}: ${t.message}`).join('\n')}

Introduce the following technical challenge in your voice (${persona.personality} teaching style):
Core topic: ${objection.coreContent}
Category: ${objection.category}
Difficulty: ${objection.severity}

Guidelines:
- Sound natural and professorial — use British English
- Match the ${objection.severity} difficulty level
- Reference specific things mentioned in the conversation if relevant
- Keep response under 50 words
- Frame it as a probing question or technical challenge, not an objection
...
Your technical challenge:`;
  ...
  private buildEvaluationPrompt(
    ...
    return `
Evaluate how well this engineering student addressed a technical challenge from their mentor.
...
${conversationContext.slice(-3).map(t => `${t.speaker === 'student' ? 'Student' : 'Mentor'}: ${t.message}`).join('\n')}
...
Analyze the response and return JSON:
{
  ...
  private getClientPersonaSystemPrompt(persona: ClientPersona): string {
    return `You are ${persona.name}, an experienced engineering mentor with a ${persona.personality} teaching style.
Background: ${persona.background}
Specialty: ${persona.budget}
Session focus: ${persona.motivations.join(', ')}
Assessment focus: ${persona.hiddenConcerns.join(', ')}

Stay in character as a knowledgeable engineering professor. Use British English. Frame all challenges as technical probing questions.`;
  }
  ...
  private generateFeedback(analysis: Record<string, unknown>, score: number): string {
    if (score >= 80) {
      return 'Excellent technical response! You demonstrated strong understanding and provided a thorough, well-structured answer.';
    } else if (score >= 60) {
      return 'Good attempt. Consider providing more specific technical details and referencing relevant standards or data.';
    } else if (score >= 40) {
      return 'The topic was partially addressed. Try to demonstrate deeper understanding by explaining the underlying principles.';
    } else {
      return 'This area needs more study. Focus on understanding the fundamental concepts before tackling complex scenarios.';
    }
  }
  ...
  private generateImprovements(analysis: Record<string, unknown>, objection: GeneratedObjection): string[] {
    ...
    if (!analysis.acknowledged) {
      improvements.push('Start by acknowledging the technical challenge and demonstrating you understand the question.');
    }
    if (!analysis.empathyShown) {
      improvements.push('Show understanding of the underlying engineering principles before jumping to solutions.');
    }
    if (!analysis.addressedDirectly) {
      improvements.push(`Address the ${objection.category.replace('_', ' ')} topic directly with specific technical details and calculations.`);
    }
    if (!analysis.askedFollowUp) {
      improvements.push('Ask a clarifying question to demonstrate deeper analysis of the problem.');
    }
  ...
  private generateResponseGuidance(
    ...
    switch (reaction.nextAction) {
      case 'accept':
        return 'The student demonstrated good understanding. The mentor acknowledges this and moves to a deeper topic.';
      case 'soften':
        return 'The student partially addressed the topic. The mentor may ask a follow-up to probe deeper understanding.';
      case 'maintain':
        return 'The student\'s understanding has gaps. The mentor will rephrase or provide hints to guide learning.';
      case 'escalate':
        return 'The student is struggling significantly. The mentor will simplify the question or provide foundational guidance.';
      default:
        return 'Continue the engineering discussion naturally.';
    }
  }
