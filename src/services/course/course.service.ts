...
export class CourseService implements ICourseService {
  ...
  async getAllCourses(): Promise<CourseListItem[]> {
    ...
    return courses.map(course => ({
      ...
      titleEn: (course as any).titleEn ?? null,
      description: course.description,
      descriptionAr: (course as any).descriptionAr ?? null,
      descriptionEn: (course as any).descriptionEn ?? null,
  ...
  async getStudentCourseProgress(traineeId: string, courseId: string): Promise<number> {
    const course = await this.courseRepository.findByIdWithLectures(courseId);
    if (!course) {
      throw new Error('Course not found');
    }
