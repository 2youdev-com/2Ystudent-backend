...
export class DiagnosticService implements IDiagnosticService {
  ...
  async buildReportFromHistory(traineeId: string): Promise<CompleteDiagnosticOutput> {
    ...
    const reports = await this.reportRepo.findByStudent(traineeId, 20);
    ...
    const recentAttempts = await this.quizRepo.findAttemptsByStudent(traineeId);
  ...
  private async extractKnowledgeGaps(traineeId: string): Promise<string[]> {
    const reports = await this.reportRepo.findByStudent(traineeId, 5);
  ...
  private skillToTopic(skill: string): string {
    const map: Record<string, string> = {
      ...
      negotiation: 'Engineering Knowledge Depth',
      objectionHandling: 'Problem Solving & Analysis',
      relationshipBuilding: 'Safety Awareness & Compliance',
      productKnowledge: 'Practical Engineering Application',
      closingTechnique: 'Critical Thinking & Reasoning',
    };
