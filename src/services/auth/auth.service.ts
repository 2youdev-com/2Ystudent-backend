...
export class AuthService implements IAuthService {
  ...
  async login(input: LoginInput): Promise<AuthResult> {
    ...
    // Get trainee's assigned teacher if any (for students)
    const traineeWithTeacher = userRole === 'student'
  ...
  async register(input: RegisterInput): Promise<AuthResult> {
    ...
    // If organizationName is provided, create a new organization and make user admin
    if (input.organizationName) {
      ...
      userRole = 'admin'; // First user of new org is admin
    }
  ...
  private mapIndustryToOrgType(industryType?: string): string {
    ...
    const industryMapping: Record<string, string> = {
      'hvac_systems': 'training_company',
      'electrical_power': 'corporate_client',
      'plc_automation': 'corporate_client',
      'industrial_maintenance': 'corporate_client',
      'building_management': 'corporate_client',
  ...
  // Normalize role for backward compatibility (user -> student, admin -> admin)
