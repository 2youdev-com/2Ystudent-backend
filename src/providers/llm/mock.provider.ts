...
export class MockLLMProvider implements ILLMProvider {
  ...
  private generateInitialMessage(personality: string): string {
    const initialMessages: Record<string, string[]> = {
      friendly: [
        "Hi there! I'm excited to discuss this engineering project. Our facility needs an upgrade and we really want to find the right solution. What can you recommend?",
        "Hello! Thanks for meeting with me. We've been looking for the right engineering partner for a while now. Where should we start?",
        "Good morning! I appreciate you taking the time. We're looking for a comprehensive engineering solution for our systems.",
      ],
      skeptical: [
        "Hi. I've worked with several engineers already and haven't been impressed. I hope you can offer something that's actually worth my time.",
        "Hello. I should tell you upfront - I've done my research and I know the technical requirements well. I won't be pressured into anything.",
        "Good morning. I'm interested in your engineering services, but I have some concerns about the proposal that I'd like to address first.",
      ],
      demanding: [
        "Good morning. I have very specific technical requirements and my time is valuable. Let's get straight to business.",
        "Hi. I need a solution that meets ALL my specifications - no compromises. Show me what you've got.",
        "Hello. I expect excellent engineering work that matches exactly what I've specified. Let's not waste time.",
      ],
      indecisive: [
        "Um, hi there. I'm looking for an engineering solution, but I'm not entirely sure what we need yet. Maybe you can help me figure that out?",
        "Hello. We're thinking about upgrading our systems, but we're also evaluating other options. We're just exploring really.",
        "Hi... sorry, I hope I'm not wasting your time. We're kind of new to this type of engineering project.",
      ],
      analytical: [
        "Good morning. Before we begin, I'd like to understand the exact specifications, efficiency ratings, and comparable project benchmarks in this field.",
        "Hello. I've prepared a list of questions about the system specifications, performance data, and technical standards. Shall we go through them?",
        "Hi. I believe in making data-driven decisions. Can you provide the technical reports and documentation on the system's performance history?",
      ],
    };
  ...
  private generatePriceResponse(personality: string, sentiment: 'positive' | 'neutral' | 'negative'): string {
    const responses: Record<string, string[]> = {
      ...
      skeptical: [
        "I've seen similar solutions quoted for less. What makes yours worth the premium?",
      ...
      analytical: [
        "Can you show me the cost per unit compared to industry benchmarks?",
        "What's the ROI potential if we factor in energy savings and reduced maintenance costs?",
        "I'd like to see a detailed cost breakdown including installation, commissioning, and annual operating costs.",
      ],
    };
  ...
  private getMockJsonResponse(prompt: string): Record<string, unknown> {
    ...
    if (lowerPrompt.includes('persona') || lowerPrompt.includes('generate')) {
      ...
      return {
        ...
        objections: [
          'Price seems high for this scope',
          'Concerned about maintenance costs',
          'The project timeline might be too long',
          'Not sure about the system specifications',
        ].slice(0, 2),
        hiddenConcerns: [
          'Worried about system reliability',
          'Uncertain about long-term plans',
          'Previous bad experience with contractors',
        ].slice(0, 2),
      };
    }
