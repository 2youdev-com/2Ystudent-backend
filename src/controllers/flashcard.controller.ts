...
export class FlashcardController {
  ...
  private initializeRoutes(): void {
    // ==========================================
    // Student routes (must come before :deckId)
    // ==========================================
  ...
  private async getAvailableDecks(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const decks = await this.flashcardService.getAvailableDecks(studentId, courseId);
  ...
  private async getStudyCards(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const studentId = req.user!.userId;
      const cards = await this.flashcardService.getStudyCards(deckId, studentId);
  ...
  private async submitReview(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const result = await this.flashcardService.submitReview(cardId, studentId, quality);
  ...
  private async getProgress(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const studentId = req.user!.userId;
      const progress = await this.flashcardService.getProgress(studentId);
  ...
  private async generateDeck(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const deck = await this.flashcardService.generateDeck(creatorId, orgId, req.body, traineeId);
      res.status(201).json(deck);
    } catch (error) {
      next(error);
    }
