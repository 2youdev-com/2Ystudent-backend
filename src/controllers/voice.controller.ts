...
export class VoiceController {
  ...
  private async startCall(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const { courseId, context, language = 'en' } = req.body;
      ...
      if (context) {
        const greetingPrompt = `You are a potential client seeking electromechanical engineering services. Start a conversation with an engineering professional. Say a short, natural opening line. Context: ${context.substring(0, 500)}`;
        ...
        try {
          ...
          greeting = 'Hello, I need help with an engineering project. Can you assist me?';
        }
      } else {
        greeting = 'Hello, I need help with an engineering project. Can you assist me?';
      }
  ...
  private async sendMessage(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const systemPrompt = `You are a potential client having a conversation with an electromechanical engineering professional.

IMPORTANT: Always respond in English only.

Your personality:
- Facility manager in your 40s
- Looking for HVAC and electrical system upgrades
- Have a reasonable budget but negotiate
- Ask realistic questions about system specifications, efficiency, and compliance
...
${session.context ? `Context: ${session.context.substring(0, 500)}` : ''}`;
      ...
      try {
        ...
        aiResponse = 'Okay, that\'s interesting. Tell me more.';
      }
  ...
  private async endCall(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      try {
        ...
        const analysisPrompt = `You are an expert at evaluating electromechanical engineering consultation performance. Analyze the conversation between an engineer and client.
...
Respond in JSON only:
{
  ...
  "breakdown": {
    ...
    "communication": <0-100>
        ...
        summary = analysis.summary || 'Conversation ended';
        feedback = analysis.feedback || 'Good overall performance';
        ...
        for (let i = 0; i < session.messages.length; i++) {
          ...
          await this.prisma.conversationTurn.create({
            data: {
              ...
              speaker: msg.role === 'user' ? 'student' : 'client',
              message: msg.content,
