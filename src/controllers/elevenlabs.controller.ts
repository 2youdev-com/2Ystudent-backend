...
export class ElevenLabsController {
  ...
  private initializeRoutes(): void {
    ...
    // Force update agent voice/conversation settings (admin only)
    this.router.post(
      '/agent/update-config',
      authMiddleware(['admin']),
      this.updateAgentConfig.bind(this)
    );
  }
  ...
  private async saveConversation(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      console.log(`[ElevenLabsController] Saving conversation ${conversationId} for student ${studentId}`);
  ...
  private async getHistory(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const studentId = req.user!.userId;
      const history = await this.elevenLabsService.getStudentConversations(studentId);
  ...
  /**
   * POST /api/elevenlabs/agent/update-config
   * Force update agent with optimized voice and conversation settings.
   * Fixes: audio cutting out, speed variations, turn detection issues.
   */
  private async updateAgentConfig(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const agentId = await this.elevenLabsService.getAgentId();
      const success = await this.elevenLabsService.updateAgentConfig(agentId);
      ...
      console.error('[ElevenLabsController] updateAgentConfig error:', error);
      next(error);
    }
  }
}
