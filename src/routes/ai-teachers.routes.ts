...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Migrate old Arabic identifiers to English display names
    const OLD_TO_NEW_NAMES: Record<string, string> = {
      ahmed: 'alex', noura: 'emily', anas: 'james', abdullah: 'robert', sara: 'lisa',
    };
    for (const teacher of teachers) {
      const newName = OLD_TO_NEW_NAMES[teacher.name];
      if (newName && teacher.isDefault) {
        await prisma.aITeacher.update({ where: { id: teacher.id }, data: { name: newName } });
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    // Normalize name the same way it will be stored
    const normalizedName = name.toLowerCase().replace(/\s+/g, '-');

    // Check for duplicate name
    const existing = await prisma.aITeacher.findFirst({
      where: { organizationId, name: normalizedName },
    });

    if (existing) {
      return res.status(400).json({ error: `A teacher with identifier "${normalizedName}" already exists (${existing.displayNameEn})` });
    }
...
router.get('/:id/available-trainees', async (req: Request, res: Response) => {
  try {
    ...
    // We show all students except those already assigned to THIS specific teacher
    const students = await prisma.trainee.findMany({
      where: {
        organizationId,
        role: 'student',
        // Exclude only students assigned to THIS teacher (by ID or legacy name)
        AND: [
    ...
    const enrichedTrainees = students.map((trainee: { id: string; firstName: string; lastName: string; email: string; currentSkillLevel: string | null; assignedTeacherId: string | null; assignedTeacher: string | null }) => {
