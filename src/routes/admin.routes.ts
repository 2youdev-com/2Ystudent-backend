...
import { IAdminService, SupervisorScope } from '../services/interfaces/admin.interface';
import { ITraineeRepository } from '../repositories/interfaces/trainee.repository.interface';

const router = Router();

// All admin routes require authentication with admin or supervisor role
// The auth middleware handles role normalization
// Note: Supervisors can view but only admin can modify users
...
function getSupervisorScope(req: Request): SupervisorScope | undefined {
...
router.get('/dashboard', async (req: Request, res: Response) => {
  try {
    ...
    const dashboard = await adminService.getDashboardData(organizationId, supervisorScope);

    // Add role info to response so frontend knows to adjust UI
    res.json({
      ...dashboard,
      userRole: req.user?.originalRole || req.user?.role,
      isTrainerView: !!supervisorScope,
    });
  } catch (error) {
...
router.get('/overview', async (req: Request, res: Response) => {
  try {
    ...
    const overview = await adminService.getOverviewStats(organizationId, supervisorScope);
    res.json(overview);
  } catch (error) {
...
router.get('/team-performance', async (req: Request, res: Response) => {
  try {
    ...
    const performance = await adminService.getTeamPerformance(organizationId, supervisorScope);
...
router.get('/trends', async (req: Request, res: Response) => {
  try {
    ...
    const trends = await adminService.getMonthlyTrends(organizationId, months, supervisorScope);
    res.json(trends);
  } catch (error) {
...
router.post('/employees', async (req: Request, res: Response) => {
  try {
    ...
    // Validate role - admin cannot create another admin or super_admin
    const allowedRoles = ['student', 'supervisor'];
    const userRole = role || 'student';
    if (!allowedRoles.includes(userRole)) {
      return res.status(400).json({
        error: 'Invalid role. Admins can only create "student" or "supervisor" accounts'
      });
    }
...
router.get('/employees/:id', async (req: Request, res: Response) => {
  try {
    ...
    const employee = await adminService.getEmployeeDetail(req.params.id, organizationId, supervisorScope);
    res.json(employee);
  } catch (error) {
...
// PATCH /api/admin/employees/:id/role - Update employee role (admin only)
router.patch('/employees/:id/role', async (req: Request, res: Response) => {
  try {
    // Only admins can change roles — supervisors must not escalate privileges
    if (req.user!.role !== 'admin') {
      return res.status(403).json({ error: 'Only admins can change employee roles' });
    }
    ...
    if (!role || !validRoles.includes(role)) {
      return res.status(400).json({ error: 'Invalid role. Must be "student", "supervisor", or "admin"' });
    }
...
router.patch('/employees/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (req.user!.role !== 'admin') {
      return res.status(403).json({ error: 'Only admins can update employee details' });
    }
...
router.get('/trainee/:traineeId/reports', async (req: Request, res: Response) => {
  try {
    ...
    const supervisorScope = getSupervisorScope(req);
    if (supervisorScope) {
      const traineeIds = await adminService.getTrainerTraineeIds(supervisorScope.trainerId, organizationId);
  ...
  try {
    ...
    if (supervisorScope) {
      ...
      const trainerAssignments = await prisma.trainerGroupAssignment.findMany({
        where: { trainerId: supervisorScope.trainerId, isActive: true },
        include: {
          group: {
      ...
      groups = trainerAssignments
        ...
        .filter(g => g.organizationId === organizationId && g.isActive !== false);
    } else {
      // Admin gets all active groups
    ...
    res.json({
      ...
      isTrainerView: !!supervisorScope,
      canCreateGroups: !supervisorScope, // Only admins can create groups
      canDeleteGroups: !supervisorScope, // Only admins can delete groups
    });
  } catch (error) {
...
router.get('/role-info', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      isTrainer: !!supervisorScope,
      isOrgAdmin: req.user?.role === 'admin',
      organizationId,
      permissions: {
        canViewAllEmployees: !supervisorScope,
        canCreateGroups: !supervisorScope,
        canDeleteGroups: !supervisorScope,
        canManageTrainers: !supervisorScope,
        canManageAdmins: !supervisorScope,
        canViewOrgWideStats: !supervisorScope,
        canModifyEmployees: req.user?.role === 'admin',
      },
    });
  } catch (error) {
